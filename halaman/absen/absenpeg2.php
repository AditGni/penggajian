<?php
$t = date('Y-m-d');
$que = mysql_query("SELECT * FROM absen WHERE nip='$id1' and tglabsen='$t'");
$k = mysql_num_rows($que);
...
<div class="col-md-7 offset-md-3">
	...
	if($k > 0){
		echo "<div class='alert alert-success'><i class='fa fa-check'></i> Anda telah selesai absen pada ".tanggal().".</div>";
	}
	?>
	<div class="card">
		...
		<div class="card-body">
			...
			<form action="?pg=proses&mod=absensi" method="POST">
				...
				<div class="form-group row">
					...
					<span class="col-md-1">:</span>
					<input type="text" name="tgl" value="<?php echo date('d-m-Y');?>" class="form-control col-md-6" readonly>
				</div>
				<button class="btn btn-primary btn-block" name="klik" type="submit" <?php echo ($k > 0) ? 'disabled' : '';?>><i class="fa fa-pencil"></i> Absen</button>
			</form>
		</div>
	</div>
</div>
